...
def process_text(doc_id, input_el, text_el):
    ...
    for div1 in input_el.findall('.FRONT/DIV1'):
        div_type = div1.attrib.get('TYPE')
        if div_type == "title page":
            TP=+1
        else:
            OtherFront=+1
    ...
    if title_date is not None:
        if year > 0:
            ...
            text_file.write('%-30s\t%s\t%s\t%s\t%d\t%d\n'  %(doc_id ,titlepage, AnotherFront, BackParatext, year, decade))                    
